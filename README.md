...
12 站联合预训练，留出站从未参与训练，全量 test 口径评测：
...
`hx` 是旧架构上的历史崩溃站点（R² 曾为 −5.94 / −7.64）。本架构在完全未见过该站
的前提下拿到 **0.9011**，是这一轮最有说服力的一条。

**适用边界**：站点按逐台功率标签的齐备程度分 A/B/C/D 四档。训练集覆盖 A/B/C 三档，
D 档在现有数据里只有一个成员，**该档的零样本能力尚未得到验证** —— 留出唯一成员
考的不是泛化，而是外推到一个空档。分档定义见 `physwm/data/registry.py`；
逐站完整评测结果见 `experiments/results/p5a_scout_full/zeroshot_eval.json`。
